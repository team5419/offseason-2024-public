...
public class AutoAlignController {
    ...
    private static final LoggedTunableNumber maxLinearVelocity =
            new LoggedTunableNumber("AutoAlign/maxLinearVelocity", SwerveConstants.kDriveConfig.maxLinearVelocity());
    private static final LoggedTunableNumber maxLinearAcceleration = new LoggedTunableNumber(
            "AutoAlign/maxLinearAcceleration", SwerveConstants.kDriveConfig.maxLinearAcceleration() * 0.4);
    private static final LoggedTunableNumber maxAngularVelocity = new LoggedTunableNumber(
            "AutoAlign/maxAngularVelocity", SwerveConstants.kDriveConfig.maxAngularVelocity() * 0.8);
    private static final LoggedTunableNumber maxAngularAcceleration = new LoggedTunableNumber(
            "AutoAlign/maxAngularAcceleration", SwerveConstants.kDriveConfig.maxAngularAcceleration() * 0.8);
    ...
    public AutoAlignController(
            Supplier<Pose2d> poseSupplier, Supplier<Translation2d> feedforwardSupplier, boolean slowMode) {
        this.poseSupplier = poseSupplier;
        this.feedforwardSupplier = feedforwardSupplier;
        this.slowMode = slowMode;
        // Set up both controllers
        linearController =
                new ProfiledPIDController(linearkP.get(), 0, linearkD.get(), new TrapezoidProfile.Constraints(0, 0));
        linearController.setTolerance(linearTolerance.get());
        thetaController =
                new ProfiledPIDController(thetakP.get(), 0, thetakD.get(), new TrapezoidProfile.Constraints(0, 0));
    ...
    private void updateConstraints() {
        if (slowMode) {
            ...
            thetaController.setConstraints(
                    new TrapezoidProfile.Constraints(slowAngularVelocity.get(), slowAngularAcceleration.get()));
        } else {
            linearController.setConstraints(
    ...
    private void resetControllers() {
        ...
        Twist2d fieldVelocity = RobotState.getInstance().getFieldVelocity();
        ...
        double linearVelocity = Math.min(
                ...
                -new Translation2d(fieldVelocity.dx, fieldVelocity.dy)
                        ...
                        .getX());
        linearController.reset(currentPose.getTranslation().getDistance(goalPose.getTranslation()), linearVelocity);
    ...
    public ChassisSpeeds update() {
        ...
        LoggedTunableNumber.ifChanged(
                hashCode(), () -> linearController.setPID(linearkP.get(), 0, linearkD.get()), linearkP, linearkD);
        LoggedTunableNumber.ifChanged(
                hashCode(), () -> thetaController.setPID(thetakP.get(), 0, thetakD.get()), thetakP, thetakD);
        LoggedTunableNumber.ifChanged(
        ...
        double driveVelocityScalar =
                linearController.getSetpoint().velocity * ffScaler + linearController.calculate(currentDistance, 0.0);
